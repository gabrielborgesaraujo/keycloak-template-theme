<#macro userProfileFormFields>
    ...
    <#list profile.attributes as attribute>
        ...
        <#if group != currentGroup>
            ...
            <#if currentGroup != "">
                ...
                >
                    ...
                    <p class="text-sm font-semibold text-gray-900">${groupHeaderText}</p>

                    <#assign groupDisplayDescription = group.displayDescription!"">
                    <#if groupDisplayDescription != "">
                        <p class="mt-2 text-sm text-gray-600">${advancedMsg(groupDisplayDescription)!""}</p>
                    </#if>
                </div>
            </#if>
        </#if>
        ...
        <div class="kc-form-group">
            <label for="${attribute.name}" class="kc-label">${advancedMsg(attribute.displayName!'')}<#if attribute.required> *</#if></label>
            <#if attribute.annotations.inputHelperTextBefore??>
                <div class="mb-2 text-sm text-gray-500" id="form-help-text-before-${attribute.name}" aria-live="polite">${kcSanitize(advancedMsg(attribute.annotations.inputHelperTextBefore))?no_esc}</div>
            </#if>
            <@inputFieldByType attribute=attribute/>
            <#if messagesPerField.existsError('${attribute.name}')>
                <span id="input-error-${attribute.name}" class="mt-1 block text-sm text-red-600" aria-live="polite">${kcSanitize(messagesPerField.get('${attribute.name}'))?no_esc}</span>
            </#if>
            <#if attribute.annotations.inputHelperTextAfter??>
                <div class="mt-2 text-sm text-gray-500" id="form-help-text-after-${attribute.name}" aria-live="polite">${kcSanitize(advancedMsg(attribute.annotations.inputHelperTextAfter))?no_esc}</div>
            </#if>
        </div>
...
<#macro inputTag attribute value>
    <input type="<@inputTagType attribute=attribute/>" id="${attribute.name}" name="${attribute.name}" value="${(value!'')}" class="kc-input"
...
<#macro textareaTag attribute>
    <textarea id="${attribute.name}" name="${attribute.name}" class="kc-input"
        aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true<#else>false</#if>"
        <#if attribute.readOnly>disabled</#if>
...
<#macro selectTag attribute>
    <select id="${attribute.name}" name="${attribute.name}" class="kc-input"
        aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true<#else>false</#if>"
        <#if attribute.readOnly>disabled</#if>
    ...
    <div class="space-y-3">
        <#list options as option>
            <label class="flex items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm text-gray-700">
                <input type="${inputType}" id="${attribute.name}-${option}" name="${attribute.name}" value="${option}" class="kc-checkbox"
                    aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true<#else>false</#if>"
                    <#if attribute.readOnly>disabled</#if>
